...
public class SysAdminServiceImpl extends ServiceImpl<SysAdminMapper, SysAdmin> implements SysAdminService {
    ...
    @Override
    public Page<SysAdmin> selectAll(Page<SysAdmin> page, SysAdmin sysAdmin) {

        return baseMapper.selectAll(page, sysAdmin);
    }
    ...
    public int updateAdmin(SysAdmin sysAdmin){
        ...
        if (res>0 && sysAdmin.getRoleId() != null) {
            SysAdminGroupAccess sysAdminGroupAccess = sysAdminGroupAccessService.getOne(
                    new LambdaQueryWrapper<SysAdminGroupAccess>().eq(SysAdminGroupAccess::getUid, sysAdmin.getId()).eq(SysAdminGroupAccess::getGroupId, sysAdmin.getRoleId()));
            // 如果没有角色，则添加角色
    ...
    @Override
    public Object administratorLogin(SysAdmin sysAdmin, HttpServletRequest request) throws IOException {
        ...
        admin.setLastLoginip(homeAddressUtil.getHomeAddressToRegion(IpAddressUtil.getIpAddress(request)));
