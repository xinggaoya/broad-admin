    ...
    <div class="nav-menus" :class="configStore.layout.layoutMode">
        <router-link class="h100" target="_blank" :title="t('home')" to="/">
            <div class="nav-menu-item">
                <Icon :color="configStore.layout.headerBarTabColor" class="nav-menu-icon" name="el-icon-Monitor"
                      size="18"/>
            </div>
        </router-link>
        <el-dropdown
        ...
        >
            ...
            <template #dropdown>
                <el-dropdown-menu class="dropdown-menu-box">
                    <el-dropdown-item v-for="item in configStore.lang.langArray" :key="item.name"
        ...
        <div @click="onFullScreen" class="nav-menu-item" :class="state.isFullScreen ? 'hover' : ''">
            <Icon
                ...
                size="18"
            />
            <Icon :color="configStore.layout.headerBarTabColor" class="nav-menu-icon" v-else name="el-icon-FullScreen"
                  size="18"/>
        </div>
        <div v-if="adminInfo.super" @click="terminal.toggle()" class="nav-menu-item pt2">
            <el-badge :is-dot="terminal.state.showDot">
                <Icon :color="configStore.layout.headerBarTabColor" class="nav-menu-icon" name="local-terminal"
                      size="26"/>
            </el-badge>
        </div>
        <el-dropdown
        ...
        >
            <div class="nav-menu-item" :class="state.currentNavMenu == 'clear' ? 'hover' : ''">
                <Icon :color="configStore.layout.headerBarTabColor" class="nav-menu-icon" name="el-icon-Delete"
                      size="18"/>
            </div>
            <template #dropdown>
                <el-dropdown-menu class="dropdown-menu-box">
