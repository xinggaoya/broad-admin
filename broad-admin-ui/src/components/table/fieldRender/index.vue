    ...
    <div v-if="field.render == 'tags'">
        <template v-if="Array.isArray(fieldValue)">
            <template v-for="tag in fieldValue">
                <el-tag v-if="tag" class="m-10" size="small" effect="light">
        ...
        <template v-for="(btn, idx) in field.buttons">
            <template v-if="btn.display ? btn.display(row, field) : true">
                ...
                >
                    ...
                    <el-button v-else v-blur @click="onButtonClick(btn.name)" :class="btn.class" class="table-operate"
                               :type="btn.type">
                        <Icon :name="btn.icon"/>
                        <div v-if="btn.text" class="table-operate-text">{{ btn.text }}</div>
                    </el-button>
                </el-tooltip>
                <el-popconfirm v-if="btn.render == 'confirmButton'" v-bind="btn.popconfirm"
                               @confirm="onButtonClick(btn.name)">
                    <template #reference>
                        <div class="ml-6">
